...
class OrderField(models.PositiveIntegerField):
	...
	def _check_for_field_attribute(self, **kwarge):
		if self.unique_for_field is None :
			return[checks.Error(" OrderField Mustbe define unique_field_attribute ")]
		elif self.unique_for_field not in [
			f.name for f in self.model._meta.get_fields() 
			]:
			return [
				checks.Error(" OrderField entered dose not match an existing model field ")
			]
		return[]
